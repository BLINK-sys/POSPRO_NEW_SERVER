...
class Warehouse(db.Model):
    ...
    # Флаг: «НДС уже в стоимости закупа?». Управляет ТОЛЬКО колонкой
    # «НДС себестоимости» в корп.расчётнике:
    #   • True  → закупочная цена уже с НДС → расчётник ИЗВЛЕКАЕТ НДС ×16/116,
    #             этот НДС можно принять к вычету при расчёте налогов.
    #   • False → закупочная цена без НДС (ИП на упрощёнке / нет входных
    #             документов) → расчётник НАКИДЫВАЕТ НДС ×16% сверху
    #             (визуально). К вычету НЕ принимается, в базе КПН
    #             закуп и доставка тоже не вычитаются.
    # Формула розницы и контрактная сторона (всегда с 16% НДС) не зависят
    # от этого флага — единая формула `(cost + delivery) × коэф_наценки`.
