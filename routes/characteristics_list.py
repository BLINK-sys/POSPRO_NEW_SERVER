...
@characteristics_list_bp.route('/api/characteristics-list', methods=['GET'])
...
@characteristics_list_bp.route('/api/characteristics-list/<int:characteristic_id>', methods=['GET'])
...
@characteristics_list_bp.route('/api/characteristics-list', methods=['POST'])
@token_required
@admin_required
def create_characteristic():
    ...
    try:
        ...
        existing = CharacteristicsList.query.filter_by(characteristic_key=data['characteristic_key']).first()
        if existing:
            return jsonify({
                'success': False,
...
@characteristics_list_bp.route('/api/characteristics-list/<int:characteristic_id>', methods=['PUT'])
@token_required
@admin_required
...
@characteristics_list_bp.route('/api/characteristics-list/<int:characteristic_id>', methods=['DELETE'])
@token_required
@admin_required
...
@characteristics_list_bp.route('/api/characteristics-list/search', methods=['GET'])
def search_characteristics():
    """Поиск характеристик по ключу"""
    try:
        ...
        characteristics = CharacteristicsList.query.filter(
            CharacteristicsList.characteristic_key.ilike(f'%{query}%')
        ).all()
        ...
        return jsonify({
            'success': False,
            'message': f'Ошибка при поиске характеристик: {str(e)}'
        }), 500
