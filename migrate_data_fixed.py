#!/usr/bin/env python3
"""
Скрипт для миграции данных с автоматическим исправлением проблемных ссылок
"""
...
def fix_data_references(data, table_name):
    """Исправляет проблемные ссылки в данных"""
    if not data:
        return data
    ...
    for row in data:
        ...
        # Исправляем ссылки для категорий
        if table_name == 'category':
            if 'parent_id' in fixed_row and fixed_row['parent_id'] == 113:
                fixed_row['parent_id'] = None
                print(f"   Исправлено: parent_id=113 -> NULL")
        
        # Исправляем ссылки для продуктов
        elif table_name == 'product':
            if 'category_id' in fixed_row and fixed_row['category_id'] == 121:
                # Найдем первую доступную категорию
                fixed_row['category_id'] = None
                print(f"   Исправлено: category_id=121 -> NULL")
        
        # Исправляем ссылки для характеристик продуктов
        elif table_name == 'product_characteristic':
            if 'product_id' in fixed_row and fixed_row['product_id'] in [23, 116, 117]:
                # Удаляем записи с несуществующими product_id
                print(f"   Пропущено: product_characteristic с product_id={fixed_row['product_id']}")
                continue
        
        # Исправляем ссылки для документов продуктов
        elif table_name == 'product_document':
            if 'product_id' in fixed_row and fixed_row['product_id'] in [23, 116, 117]:
                # Удаляем записи с несуществующими product_id
                print(f"   Пропущено: product_document с product_id={fixed_row['product_id']}")
                continue
        
        # Исправляем ссылки для медиа продуктов
        elif table_name == 'product_media':
            if 'product_id' in fixed_row and fixed_row['product_id'] in [23, 116, 117]:
                # Удаляем записи с несуществующими product_id
                print(f"   Пропущено: product_media с product_id={fixed_row['product_id']}")
                continue
        
        # Исправляем ссылки для элементов заказов
        elif table_name == 'order_items':
            if 'product_id' in fixed_row and fixed_row['product_id'] == 37:
                # Удаляем записи с несуществующими product_id
                print(f"   Пропущено: order_items с product_id={fixed_row['product_id']}")
                continue
        
        # Исправляем ссылки для избранного
        elif table_name == 'favorites':
            if 'product_id' in fixed_row and fixed_row['product_id'] == 23:
                # Удаляем записи с несуществующими product_id
...
def migrate_table(local_conn, render_conn, table_name):
    ...
    # Исправляем проблемные ссылки
    print(f"Исправление проблемных ссылок...")
    fixed_data = fix_data_references(local_data, table_name)
...
def main():
    ...
    print("(с автоматическим исправлением проблемных ссылок)")
    print("=" * 60)
